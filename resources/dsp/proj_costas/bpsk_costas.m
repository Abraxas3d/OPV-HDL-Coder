...
for i = 1:N
  ...
  if (_baud_timer >= 1/baudrate)
    _current_symbol = rand() > 0.5;
    _baud_timer -= 1/baudrate;
  endif
  
  carrier = cos(2 * pi * _time * (1 + Cppm) * Cfreq + deg2rad(Cphse)); 
  
  signal_baseband(i)  = _current_symbol;
  signal_modulated(i) = carrier * (-1 + 2*_current_symbol) + randn(1) / 100;
endfor;
...
filter_loop = PIController(20, 0.1, Fs);
nco         = NCO(Fs);
control     = 0;
...
for i = 1:N
  ...
  control = filter_loop.update(branch_I * branch_Q);
